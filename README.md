...
2. Output Word filename (default: `output.docx`)
3. Zoom factor for image quality (default: `2.5`)
...
1. **Opens PDF**: Validates and loads the PDF file using PyMuPDF
2. **Skips First Page**: Excludes the cover/first page from conversion
3. **Renders Pages**: Converts each remaining page to PNG images at specified zoom level
4. **Creates Word Document**: Generates a landscape-oriented Word document with minimal margins
5. **Arranges Layout**: Places two page images per row in a table structure
6. **Adds Page Numbers**: Labels each image with its original page number
7. **Cleans Up**: Removes temporary image files after successful conversion
...
**Note**: For text-editable Word documents, consider OCR tools if the PDF contains selectable text.
